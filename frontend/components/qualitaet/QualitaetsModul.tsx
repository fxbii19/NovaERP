...
export default function QualitaetsModul({ modus }: { modus: QmModus }) {
  ...
  const [ausgewaehlterAuftragId, setAusgewaehlterAuftragId] = useState<number | null>(null);
  ...
  {!laedt && daten && modus === "pruefauftraege" && <><form onSubmit={(e) => { e.preventDefault(); void senden("pruefauftrag-anlegen"); }} className="mt-8 grid gap-4 rounded-2xl border border-[var(--nova-rand)] bg-[var(--nova-flaeche)] p-5 md:grid-cols-3"><Auswahl label="Artikel" wert={formular.artikelId ?? ""} onChange={(v) => setFormular((a) => ({ ...a, artikelId: v }))} optionen={artikel.map((a) => [String(a.id), `${a.artikelnummer} · ${a.produktname}`])} /><Auswahl label="Lagerplatz" erforderlich={false} wert={formular.lagerplatzId ?? ""} onChange={(v) => setFormular((a) => ({ ...a, lagerplatzId: v }))} optionen={lagerplaetze.map((l) => [String(l.id), `${l.code} · ${l.bezeichnung}`])} /><Auswahl label="Prüfart" wert={formular.typ ?? ""} onChange={(v) => setFormular((a) => ({ ...a, typ: v }))} optionen={[["EINGANGSPRUEFUNG", "Eingangsprüfung"], ["ZWISCHENPRUEFUNG", "Zwischenprüfung"], ["ENDPRUEFUNG", "Endprüfung"]]} /><Feld label="Prüfmenge" typ="number" wert={formular.pruefmenge ?? ""} onChange={(v) => setFormular((a) => ({ ...a, pruefmenge: v }))} /><Auswahl label="Priorität" wert={formular.prioritaet ?? "NORMAL"} onChange={(v) => setFormular((a) => ({ ...a, prioritaet: v }))} optionen={[["NIEDRIG", "Niedrig"], ["NORMAL", "Normal"], ["HOCH", "Hoch"], ["DRINGEND", "Dringend"]]} /><Feld label="Zuweisen an" erforderlich={false} wert={formular.zugewiesenAn ?? ""} onChange={(v) => setFormular((a) => ({ ...a, zugewiesenAn: v }))} /><div className="md:col-span-3"><Primaer text="Prüfauftrag anlegen" /></div></form><AuftragsTabelle auftraege={daten.pruefauftraege} onAuswaehlen={setAusgewaehlterAuftragId} />{daten.pruefauftraege.find((a) => a.id === ausgewaehlterAuftragId && a.status === "OFFEN") && <div className="mt-6"><h2 className="mb-3 text-xl font-semibold">Prüfung bearbeiten</h2><PruefFormular auftrag={daten.pruefauftraege.find((a) => a.id === ausgewaehlterAuftragId)!} onSenden={async (werte) => { const ok = await senden("pruefung-abschliessen", { pruefauftragId: ausgewaehlterAuftragId, ...werte }); if (ok) setAusgewaehlterAuftragId(null); return ok; }} /></div>}{daten.pruefauftraege.find((a) => a.id === ausgewaehlterAuftragId && a.status === "FREIGABE_OFFEN") && <div className="mt-6"><h2 className="mb-3 text-xl font-semibold">Freigabe entscheiden</h2><FreigabeKarte auftrag={daten.pruefauftraege.find((a) => a.id === ausgewaehlterAuftragId)!} onEntscheiden={async (entscheidung, begruendung) => { const auftrag = daten.pruefauftraege.find((a) => a.id === ausgewaehlterAuftragId)!; const ok = await senden("freigabe-entscheiden", { pruefungId: auftrag.pruefung?.id, entscheidung, begruendung }); if (ok) setAusgewaehlterAuftragId(null); return ok; }} /></div>}</>}
...
function AuftragsTabelle({ auftraege, onAuswaehlen }: { auftraege: Pruefauftrag[]; onAuswaehlen: (id: number) => void }) { return <div className="mt-6 overflow-auto rounded-2xl border border-[var(--nova-rand)]"><table className="w-full text-sm"><thead className="bg-[var(--nova-flaeche)]"><tr>{["Prüfnummer", "Artikel", "Art", "Menge", "Priorität", "Status", "Aktion"].map((t) => <th key={t} className="px-4 py-3 text-left">{t}</th>)}</tr></thead><tbody>{auftraege.map((a) => { const bearbeitbar = a.status === "OFFEN" || a.status === "FREIGABE_OFFEN"; return <tr key={a.id} onClick={() => bearbeitbar && onAuswaehlen(a.id)} className={`border-t border-[var(--nova-rand)] ${bearbeitbar ? "cursor-pointer transition hover:bg-[var(--nova-flaeche-hover)]" : ""}`}><td className="px-4 py-3 text-[var(--nova-akzent)]">{a.pruefnummer}</td><td className="px-4 py-3">{a.artikel.artikelnummer}</td><td className="px-4 py-3">{a.typ}</td><td className="px-4 py-3">{zahl(a.pruefmenge)}</td><td className="px-4 py-3">{a.prioritaet}</td><td className="px-4 py-3">{a.status}</td><td className="px-4 py-3">{bearbeitbar ? <button type="button" onClick={(e) => { e.stopPropagation(); onAuswaehlen(a.id); }} className="rounded-lg bg-[var(--nova-akzent)] px-3 py-2 font-semibold text-white">{a.status === "OFFEN" ? "Prüfen" : "Freigeben"}</button> : <span className="text-emerald-400">Erledigt</span>}</td></tr>; })}</tbody></table></div>; }
