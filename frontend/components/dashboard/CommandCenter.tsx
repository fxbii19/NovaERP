...
export default function CommandCenter() {
  ...
  return <div className="mt-8 space-y-8">
    ...
    <section><h2 className="mb-4 text-xl font-semibold">Offene Aufgaben</h2><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {[['MDE bestätigen', daten.kennzahlen.offeneMde, '/lager/produktzugang'], ['QS bearbeiten', daten.kennzahlen.offeneQs, '/qualitaet/pruefauftraege'], ['Inventurdifferenzen', daten.kennzahlen.offeneInventuren, '/lager/inventur'], ['Sperrbestand prüfen', daten.kennzahlen.sperrbestaende, '/qualitaet/sperrbestand'], ['Bestandsrisiken', daten.kennzahlen.kritischeBestaende, '/bestand'], ['Artikel ohne Bestand', daten.kennzahlen.ohneBestand, '/bestand']].map(([label, wert, href]) => <Link key={String(label)} href={String(href)} className="flex items-center justify-between rounded-xl border border-[var(--nova-rand)] bg-[var(--nova-flaeche)] px-4 py-4 transition hover:border-[var(--nova-akzent)]"><span>{label}</span><strong>{Number(wert).toLocaleString('de-DE')}</strong></Link>)}</div></section>
